...
Float_t A2    = 1.0;     // polar distribution
Float_t A4    = 0.0;
...
//Float_t enhance = 1.;           // enhancement factor
//Float_t enhancepi=40.;            //enhancement factor for pi
...
PFireball *source9=new PFireball("phi",Eb,T1,T2,frac,blast,0.0,A4,v1,v2);
PFireball *source10=new PFireball("pi-",Eb,T1,T2,frac,blast,A2,A4,v1,v2);
PFireball *source11=new PFireball("p",Eb,T1,T2,frac,blast,0.0,A4,v1,v2);
...
//
 ...
 cout <<source1 ->IsRandomB() << endl;
 // return;

 for (int i=0;i<100000;i++) {

     mean_pi+=source1->sampleNProd();
     mean_eta+=source2->sampleNProd();

 }

 cout << mean_pi << ":" <<  mean_eta << endl;
  cout << mean_pi / mean_eta << endl;
  return;
#endif
...
//PReaction *r=new PReaction(cc,"delme",21,0,0,0,1);
//PReaction *r=new PReaction(cc,"delme",1,1,0,0,1);
PReaction *r=new PReaction(cc,"delme",1,0,0,0,1);
...
//r->setTrigCond(1);    // require at least 1 lepton per event
 ...
 TH1F * histo = new TH1F ("histo","M_ee",100,0.,1.2);
 //r->Do(histo,"_x= ([e+] + [e-])->M();");
  r->Do(histo,"_x= ([g])->E();"); //BUGBUG: Does not skip empty events
