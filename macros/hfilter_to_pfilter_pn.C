{
    ...
    TFile *f = new TFile("pluto_ee_filter_may07.root","RECREATE");
    ...
    l->AddCommand(efficiency_eminus,"eff_em = Eval(); if (!eff_em); [e-]->SetInActive();if (_filter_debug); echo removed e- (eff); if (_filter_debug); echo removed e- (eff)");
    ...
    l->AddCommand(efficiency_eplus,"eff_ep = Eval(); if (!eff_ep); [e+]->SetInActive();if (_filter_debug); echo removed e+ (eff); if (_filter_debug); echo removed e+ (eff)");
