{
    ...
    TFile *filters_ep = new TFile("../../matricesEffSingle.PairCode.all.cuts.ep.apr06.root");
    ...
    TFile *filters_ep = new TFile("../../matricesEffSingle.PairCode.all.cuts.em.apr06.root");
    ...
    TFile *f = new TFile("pluto_ee_filter_apr06.root","RECREATE");

    PCommandList *p = new PCommandList("_main", "echo **** HADES ee acceptance/efficiency filter for Apr06");
    ...
    p->AddCommand("echo Usage:      1.) as a particle remover <default>:");
    p->AddCommand("echo Usage:      2.) the event filter mode (with acc*eff):");
    p->AddCommand("echo Usage:          makeDistributionManager()->Startup(\"_filter_remove_particles=0\");");    
    p->AddCommand("echo Usage:          and, e.g.: #eefilter=bool_eff");
    p->AddCommand("echo ********************************************************************************");
    p->AddCommand("echo Debug mode:     makeDistributionManager()->Startup(\"_filter_debug=1\");");
    p->AddCommand("echo ********************************************************************************");
    p->AddCommand("_filter_debug=0");
    p->AddCommand("_filter_remove_particles=1");
    ...
    s->AddCommand("echo This event loop uses the HADES ee acceptance/efficiency filter for Apr06 ");
    s->AddCommand("echo ********************************************************************************");
    s->Write();
    ...
    l->AddCommand("_y = [e-]->Theta(); ");
    l->AddCommand("_z = [e-]->P() * 1000.; if ([e-]->P() > 1.8) ; _z = 1800");
    //l->AddCommand(efficiency_eminus,"eff_em = Eval(); [e-]->SetInActive(); ; if (eff_em >0); [e-]->SetActive();");
    l->AddCommand(efficiency_eminus,"eff_em = Eval(); if (!eff_em); [e-]->SetInActive();");
    ...
    l->AddCommand("_z = [e+]->P() * 1000.; if ([e+]->P() > 1.9) ; _z = 1900");
    //l->AddCommand("_z = 0.; ");
    //l->AddCommand(efficiency_eplus,"eff_ep = Eval(); [e+]->SetInActive(); ; if (eff_ep >0); [e+]->SetActive();");
    l->AddCommand(efficiency_eplus,"eff_ep = Eval(); if (!eff_ep); [e+]->SetInActive();");
