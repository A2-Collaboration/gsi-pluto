...

{
    ...
    //source1->SetRapidityDistribution(new TF1("f","1+x",-1,2));
    TF1 *fg41 = new TF1("fg41","gaus(0)+gaus(6)",-2,2);
    fg41->SetFillColor(19);
    fg41->SetFillStyle(0);
    fg41->SetLineWidth(3);
    fg41->SetLineStyle(2);
    fg41->SetLineColor(2);
    fg41->SetChisquare(3427.51);
    fg41->SetNDF(18);
    fg41->SetParameter(0,1.107502);
    fg41->SetParError(0,0.02383138);
    fg41->SetParLimits(0,0,0);
    fg41->SetParameter(1,-0.5873293);
    fg41->SetParError(1,0.003554181);
    fg41->SetParLimits(1,0,0);
    fg41->SetParameter(2,0.3482508);
    fg41->SetParError(2,0.001760564);
    fg41->SetParLimits(2,0,0);
    fg41->SetParameter(6,1.107502);
    fg41->SetParError(6,0.02390824);
    fg41->SetParLimits(6,0,0);
    fg41->SetParameter(7,0.5873293);
    fg41->SetParError(7,0.00356595);
    fg41->SetParLimits(7,0,0);
    fg41->SetParameter(8,0.3482508);
    fg41->SetParError(8,0.001770609);
    fg41->SetParLimits(8,0,0);
    source1->SetRapidityDistribution(fg41);

    PChannel *c1 = source1->makeChannel(20, Mpi0);          // pi0 decay directly
    source1->Print();

    PReaction *r=new PReaction(&c1,"thermal_test",1);
