...
TH1F * pp_inc2 = new TH1F ("pp_inc","pp DiLepton mass (incoherent sum)",histo_bins,0.1,histo_max);
pp_inc2->Sumw2();
...
//Dilepton mass
m3->AddHistogram(pp_inc2,"_x=[dilepton]->M()");
...
PReaction *my_reaction3a = new PReaction("1.25","p","p","p p dilepton [e+ e-]",NULL,1,0,0,0);
TH1F * pp_el2 = new TH1F ("pp_el","pp DiLepton mass (elastic)",histo_bins,0.1,histo_max);
pp_el2->Sumw2();
...
//Dilepton mass
m3a->AddHistogram(pp_inc2,"_x=[dilepton]->M()");
m3a->AddHistogram(pp_el2,"_x=[dilepton]->M()");

my_reaction3a->AddBulk(m3a);
my_reaction3a->Loop(num_events);


PUtils::correct(pp_inc2); //correct for number of used bins
PUtils::correct(pp_el2);
