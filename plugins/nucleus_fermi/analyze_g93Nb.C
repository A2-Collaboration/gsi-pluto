{
  ...
  TCanvas *can = new TCanvas("can","g+93Nb->Pi0+Eta+n+92Nb",200,10,600,400);
  ...
  double px,py,pz;
  TH1F *h1 = new TH1F("h1","Distribution of Fermi Momentum inside Niobium",1000,0.,1.);
  makeDistributionManager()->Exec("nucleus_fermi:gamma");
  makeDistributionManager()->LinkDB();
  PFermiMomentumGA* f1 = 
      ((PFermiMomentumGA*)makeDistributionManager()->GetDistribution("gn_in_93Nb"));   
  ...
  can->Print("g93Nb_BreakUp.eps");
  can->Print("g93Nb_BreakUp.root");
