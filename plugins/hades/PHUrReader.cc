...
Bool_t PHUrReader::ReadEvent()
{
	...
	while ( !fInputAscii.eof() ){
		...
		PHUrParticle part;
		if(!part.Read(fInputAscii)) { Error("ReadEvent()","Could not read in-going particle!"); return kFALSE; }
		...
		PHUrParticle part;
		if(!part.Read(fInputAscii)) { Error("ReadEvent()","Could not read out-going particle!"); return kFALSE; }
...
Bool_t PHUrReader::Modify(PParticle ** mstack, int *decay_done, int * num, int stacksize) {
	...
	for(UInt_t iout = 0; iout <  collisionsOut[icol].size(); iout++)  // out-going particles
	{
		...
		for(UInt_t iin = 0; iin < collisionsIn[icol2].size(); iin++)    // in-going particles
		{
			 ...
			 if (particleIn.id == particleOut.id && particleIn.Pz() == particleOut.Pz())  // same particle in following collision
			 {
				...
				particleIn.id == dilep.phi) {  // dilepton interest
				 ...
				 if(!outputFreezeoutDiLeptons) { dilep.Dilep(); }
				 else    flag = 0;             // not final state
		...
		if( !(particleOut.id == dilep.omega ||
			...
			if(outputNonDiLeptons==1){
			    ...
			    pMoth->Reset(particleOut.pdg,particleOut.Px(),particleOut.Py(),particleOut.Pz(),particleOut.E(), 1.);
