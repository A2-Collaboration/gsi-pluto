...
//     cout << cur << ":" << stacksize<< endl;
//     cout << npart << ":" << evnr <<":" << stepnr << endl;
	...
	if (! makeDataBase()->GetParamInt (pdg_param, part->GetPdg() , pid_param, &i_result)) {
		...
		if (unknown_pdg[i] == part->GetPdg()) found=kTRUE;
	    }
	    if (!found) {
		Warning("Modify",
			"Particle with PDG code %i is not defined in Pluto, further particles will get the 'dummy' pid",part->GetPdg());
